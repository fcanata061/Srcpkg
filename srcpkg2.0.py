...
# -*- coding: utf-8 -*-
"""
srcpkg.py — gerenciador source-based (protótipo completo, revisado)
...
- Corrigido bug em cmd_orphans (flag --remove agora funciona).
- Melhoria no suporte a fontes git (usa source.url OU git.repo, se presente).
- Verificações e mensagens mais claras quando ferramentas externas estão ausentes
  (patch/ldd/tar/unzstd/7z/strip).
- Validações extras em PackageMeta.from_json.
- Pequenos aprimoramentos de robustez no runner e em apply_patches/extract.
- NOVO: comando `doctor` para checar dependências do sistema.
- NOVO: instalação com `--root` (staging) e `--allow-nonroot` para facilitar uso com `fakeroot`.
"""
...
def run(cmd:List[str]|str, cwd:Optional[Path]=None, env:Optional[Dict[str,str]]=None, check=True, logfile:Optional[Path]=None):
    ...
    f=None
    try:
        proc = subprocess.Popen(args, cwd=str(cwd) if cwd else None, env=env, shell=shell,
...
class SourceSpec:
    url:Optional[str]=None; sha256:Optional[str]=None; type:str="archive"

@dataclasses.dataclass
class PatchSpec:
    url:Optional[str]=None; file:Optional[str]=None; sha256:Optional[str]=None; strip:int=1
...
class PackageMeta:
    ...
    def from_json(path:Path)->"PackageMeta":
        try:
            ...
            raise ValueError(f"Falha ao ler receita '{path}': {e}")
        if "name" not in data or "version" not in data:
            raise ValueError(f"Receita '{path}' precisa de 'name' e 'version'")
        src=data.get("source",{})
        source=SourceSpec(url=src.get("url"), sha256=src.get("sha256"), type=src.get("type","archive"))
        patches=[]
        for p in data.get("patches",[]):
            try:
                patches.append(PatchSpec(url=p.get("url"), file=p.get("file"), sha256=p.get("sha256"), strip=int(p.get("strip",1))))
            except Exception:
                patches.append(PatchSpec(url=p.get("url"), file=p.get("file"), sha256=p.get("sha256"), strip=1))
        b=data.get("build",{})
        build=BuildSpec(env=b.get("env",{}), prepare=b.get("prepare",[]), compile=b.get("compile",[]), install=b.get("install",[]))
...
class SrcPkg:
    ...
    def extract(self, archive_or_dir:Path, workdir:Path)->Path:
        ...
        if name.endswith((".tar.gz",".tgz",".tar.bz2",".tbz2",".tar.xz",".txz",".tar.zst")):
            if name.endswith(".tar.zst"):
                if not which("tar") or not which("unzstd"):
                    raise RuntimeError("Para .tar.zst, instale 'tar' e 'unzstd'.")
    ...
    def apply_patches(self, meta:PackageMeta, srcdir:Path, patch_dir:Path):
        ...
        if not which("patch"):
            raise RuntimeError("Ferramenta 'patch' não encontrada; instale 'patch'.")
    ...
    def _strip_files(self, root:Path):
        if not which('strip'):
            log_warn("'strip' não encontrado; pulando strip.")
            return
        for p in root.rglob('*'):
    ...
    def install_package_dir(self, meta:PackageMeta, destdir:Path, target_root:Path=Path('/'), allow_nonroot:bool=False)->InstalledPkg:
        if os.geteuid()!=0 and (str(target_root)=='/' and not allow_nonroot):
            log_err('Requer root para instalar no /. Use sudo ou passe --root para staging / --allow-nonroot.'); sys.exit(1)
        files=[str(p) for p in self._copy_tree(destdir, target_root)]
    ...
    def build(self, meta:PackageMeta, work:Path, only_build:bool=True)->Tuple[Path,Path]:
        ...
        # escolher a fonte correta (git ou arquivo)
        if meta.source.type=='git' or meta.git.get('repo'):
            src_spec = SourceSpec(url=(meta.source.url or meta.git.get('repo')), type='git')
        else:
            src_spec = meta.source
        src_path=self.download(src_spec, work)
    ...
    def install_from_meta(self, meta:PackageMeta, target_root:Path=Path('/'), allow_nonroot:bool=False):
        ...
        self.install_package_dir(meta, destdir, target_root=target_root, allow_nonroot=allow_nonroot)
    ...
    def revdep(self, rebuild:bool=False):
        if not which("ldd"):
            log_warn("'ldd' não encontrado; revdep indisponível neste sistema.")
            return
    ...
    def _dict_to_meta(self, d:Dict[str,object])->PackageMeta:
        ...
        b=d.get("build",{})
        build=BuildSpec(**{k:b.get(k,[]) if k in ("prepare","compile","install") else b.get(k,{}) for k in ["env","prepare","compile","install"]})
...
def cmd_build(a):
    m=SrcPkg(); meta=load_meta_from_arg(a.recipe); work=SRCPKG_BUILD/f"{meta.name}-{meta.version}"
...
def cmd_install(a):
    m=SrcPkg(); meta=load_meta_from_arg(a.recipe)
    m._resolve_deps_install_first(meta)
    root = Path(a.root) if a.root else Path('/')
    m.install_from_meta(meta, target_root=root, allow_nonroot=a.allow_nonroot)
...
def cmd_orphans(a):
    m=SrcPkg(); orfs=m.orphans(); print("Órfãos:"); [print(f"  - {n}") for n in orfs]
    if a.remove:
...
def cmd_doctor(a):
    checks = [
        ...
        ("wget/curl", which('wget') is not None or which('curl') is not None),
        ("patch", which('patch') is not None),
        ("tar", which('tar') is not None),
        ("unzstd (para .tar.zst)", which('unzstd') is not None),
        ("7z (para .7z)", which('7z') is not None),
        ("strip (opcional)", which('strip') is not None),
        ("ldd (revdep)", which('ldd') is not None),
    ]
    ok_all=True
    print(c("Checagens do sistema:", BOLD))
    for name, ok in checks:
        print(f" - {name}: {'OK' if ok else 'FALTA'}")
        if not ok and name in {"wget/curl","patch","tar"}: ok_all=False
    if os.geteuid()!=0:
        print(" - permissão: usuário comum (ok para build/package; use --root para staging ou sudo para instalar em /)")
    else:
        print(" - permissão: root")
    if not ok_all:
        sys.exit(2)
...
def make_parser()->argparse.ArgumentParser:
    ...
    sp=sub.add_parser('install'); sp.add_argument('recipe'); sp.add_argument('--root', help='instalar em raiz alternativa (ex.: ./staging)'); sp.add_argument('--allow-nonroot', action='store_true', help='permitir instalar sem root (útil com fakeroot + --root)'); sp.set_defaults(func=cmd_install)
